	...
	<div class="row">
			...
			<tbody>
				<template v-for="(student, index) in computed_result" :key="student.id">
					<tr :class="{ success: student.rank > 0 && student.rank < 4 }">
						...
						<td v-for="mark in student.total_obtain_marks">@{{ Number((mark).toFixed(2)) }}</td>

						<td>@{{ Number((student.total_obtain_marks_sum).toFixed(2)) }}</td>
		...
		<table class="table" v-if="computed_result.length">
			<tbody>
				<tr>
					<td>
						{{-- hide because like one position can have multiple students --}}
						{{-- <template v-if="false"> 
							<u>
		...
		methods: {
			...
			compute_result() {
				...
				this.results[mainIdx]?.forEach(result => {
					...
					student.percentage = ((student.total_obtain_marks_sum / student.total_marks_sum) * 100).toFixed(1);
