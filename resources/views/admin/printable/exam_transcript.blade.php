...
@section('content')
<div class="container-fluid" style="border: 1px solid black; padding: 10px; margin: 10px" >

        <div style="">

            <img alt="image" src="{{ tenancy()->tenant->system_info['general']['logo'] ? route('system-setting.logo') : URL('/img/logo-1.png')}}" style="position: absolute; top: 20px; left: 22px; height: 8em;">
            
            <h2 class="text-center">{{ tenancy()->tenant->system_info['general']['title'] }}</h2>    
            <h3 class="text-center"><u>@{{ exam_title }} Result @{{ selected_exams[0].academic_session.title }}</u></h3>
        ...
        <div>
            ...
            <table style="width: 100%; margin-bottom: 10px">
                <tbody>
                    <tr>
                        <td><h4>Student Name: <u>@{{ student.name }}</u></h4></td>
                        <td><h4>Class: <u>@{{ student_class.prifix }}</u></h4></td>
                    </tr>
                    <tr>
                        <td><h4>Father Name: <u>@{{ student.father_name }}</u></h4></td>
                        <td><h4>G.R.No: <u>@{{ student.gr_no }}</u></h4></td>
                        {{-- <td><h4>Attendance: <u>@{{ StudentAttendance(attendance.total) + '/' + attendance.total.length }}</u></h4></td> --}}
                    </tr>
                </tbody>
            </table>
        </div>
...
<script type="text/javascript">
	var app = new Vue({
		...
		data: { 
            ...
            tenantLogo: '{{ tenancy()->tenant->system_info['general']['logo'] ? route('system-setting.logo') : URL('/img/logo-1.png')}}'
		},
		mounted: function(){
